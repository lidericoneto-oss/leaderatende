  ...
  return (
    <div className="min-h-screen bg-dark">
      <header className="mx-auto flex max-w-6xl items-center justify-between px-5 py-6">
        ...
        <Link
          href="/diagnostico"
          className="rounded-lg border border-dark-border px-4 py-2 text-sm font-medium text-white transition hover:border-brand hover:text-brand"
        >
          Iniciar avaliação
        </Link>
      </header>
      ...
      <main>
        <section className="mx-auto max-w-4xl px-5 pt-10 pb-20 text-center sm:pt-16 sm:pb-28">
          <span className="inline-flex items-center rounded-full border border-dark-border bg-dark-surface px-3 py-1 text-xs font-medium text-dark-ink-soft">
            Avaliação gratuita · Leva menos de 5 minutos
          </span>
          <h1 className="font-display mt-6 text-4xl leading-[1.05] tracking-tight text-white uppercase sm:text-6xl">
            Descubra como sua empresa está sendo{" "}
            <span className="text-brand">percebida no digital.</span>
          </h1>
          <p className="mx-auto mt-6 max-w-2xl text-base text-dark-ink-soft sm:text-lg">
          ...
          <div className="mt-9 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
            <Link
              href="/diagnostico"
              className="w-full rounded-lg bg-brand px-7 py-3 text-sm font-semibold text-white transition hover:bg-brand-dark sm:w-auto"
            >
              Fazer minha avaliação agora
            </Link>
            <span className="text-xs text-dark-ink-soft">
        ...
        <section className="border-y border-dark-border bg-dark-surface">
          <div className="mx-auto max-w-5xl px-5 py-14 sm:py-20">
            ...
            <h2 className="font-display mt-2 max-w-xl text-2xl tracking-tight text-white uppercase sm:text-3xl">
            ...
            <ul className="mt-8 grid gap-4 sm:grid-cols-2">
              {AUDIENCE.map((item) => (
                <li
                  key={item}
                  className="flex items-start gap-3 rounded-xl border border-dark-border bg-dark p-4 text-sm text-dark-ink-soft"
                >
          ...
          <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {STEPS.map((s) => (
              <div key={s.number}>
                ...
                <p className="mt-1.5 text-sm text-dark-ink-soft">
                  {s.description}
                </p>
              </div>
            ))}
          ...
          <div className="mx-auto max-w-5xl px-5 py-14 sm:py-20">
            ...
            <div className="mt-8 flex flex-wrap gap-2.5">
              {PILLARS.map((p) => (
                <span
                  key={p}
                  className="rounded-full border border-dark-border bg-dark px-4 py-2 text-sm font-medium text-white"
                >
                  {p}
                </span>
              ))}
            </div>
            <p className="mt-6 max-w-2xl text-sm text-dark-ink-soft">
        ...
        <section className="mx-auto max-w-3xl px-5 py-16 text-center sm:py-24">
          <h2 className="font-display text-2xl tracking-tight text-white uppercase sm:text-4xl">
            Faça uma avaliação rápida e descubra onde estão os principais
            pontos de melhoria.
          </h2>
          <Link
            href="/diagnostico"
            className="mt-8 inline-block rounded-lg bg-brand px-7 py-3 text-sm font-semibold text-white transition hover:bg-brand-dark"
          >
      ...
      <footer className="border-t border-dark-border px-5 py-8 text-center text-xs text-dark-ink-soft">
        <p>
